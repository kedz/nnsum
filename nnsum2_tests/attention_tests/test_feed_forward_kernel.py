...
def test_is_stateless(hidden_size):
    kernel = nnsum2.attention.FeedForwardKernel(hidden_dims=hidden_size)
    assert kernel.is_stateless == True

def test_broadcast_add(key, query, batch_size, key_length, query_length,
                       hidden_size):

    ref = torch.FloatTensor(query_length, batch_size, key_length, hidden_size)
    for q in range(query_length):
        for b in range(batch_size):
            for k in range(key_length):
                ref[q,b,k] = key[b,k] + query[q,b]
    actual = nnsum2.attention.FeedForwardKernel._broadcast_add(
        None, key, query)

    assert torch.allclose(ref, actual)

def test_sequential_matches_batch(key, query, key_mask, batch_size, 
                                  key_length, query_length, hidden_size):
    module = nnsum2.attention.FeedForwardKernel(hidden_dims=hidden_size)
...
def test_key_masking_gradient(key, query, key_mask, hidden_size):
    module = nnsum2.attention.FeedForwardKernel(hidden_dims=hidden_size)
    batch_attention, batch_state = module(key, query, key_mask=key_mask)
...
def test_masking_output(key, query, key_mask, hidden_size):
    module = nnsum2.attention.FeedForwardKernel(hidden_dims=hidden_size)
    batch_attention, batch_state = module(key, query, key_mask=key_mask)
    assert batch_state is None
