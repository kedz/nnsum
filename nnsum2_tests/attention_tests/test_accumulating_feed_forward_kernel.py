...
def test_is_stateless(hidden_size):
    kernel = nnsum2.attention.AccumulatingFeedForwardKernel(
...
def test_broadcast_add(key, query, batch_size, key_length, query_length,
    ...
    actual = nnsum2.attention.AccumulatingFeedForwardKernel._broadcast_add(
        None, key, query)
...
def test_masking_output(key, query, key_mask, init_accumulator, hidden_size):
    kernel = nnsum2.attention.AccumulatingFeedForwardKernel(
        hidden_dims=hidden_size)
    batch_attention, batch_state = kernel(key, query, key_mask=key_mask,
                                          state=init_accumulator)
...
def test_limiting_attention(key, query, key_length, hidden_size):
