...
from model_util import chen_and_lapata_sent_model
...
def main():
    ...
    parser.add_argument(
        "--filter-windows", default=[1, 2, 3], type=int, nargs="+")
    parser.add_argument(
        "--feature-maps", default=[25, 25, 25], type=int, nargs="+")
    parser.add_argument(
        "--sentence-dropout", default=.25, type=float)
    parser.add_argument(
        "--rnn-hidden-size", default=100, type=int)
    parser.add_argument(
        "--sentence-rnn-hidden-size", default=100, type=int)
    parser.add_argument(
...
#    parser.add_argument("--shuffle-doc", default=False, action="store_true")
    ...
    parser.add_argument(
        "--sentence-encoder", default="cnn", choices=["cnn", "avg", "rnn"])
    parser.add_argument(
        "--sentence-extractor", default="c&l", 
        choices=["c&l", "simple", "rnn"])
    parser.add_argument("--attention", default=None, choices=["dot"])
    parser.add_argument(
        "--rnn-encoder-bidirectional", action="store_true", default=False)

    args = parser.parse_args()

    if len(args.feature_maps) != len(args.filter_windows):
        print("--feature-maps and --filter-windows must have same number",
              "of arguments!")
        sys.exit(1)
    ...
    model = chen_and_lapata_sent_model(
        embeddings,
        ...
        rnn_encoder_bidirectional=args.rnn_encoder_bidirectional,
        mlp_layers=args.mlp_layers,
        mlp_dropouts=args.mlp_dropouts,
        sentence_encoder=args.sentence_encoder,
        sentence_extractor=args.sentence_extractor,
