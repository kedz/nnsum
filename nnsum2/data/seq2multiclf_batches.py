...
@Parameterized.register_object("data.seq2multiclf_batches")
class Seq2MultiClfBatches(Parameterized):
    ...
    def place_batch_on_gpu(self, batch):
        ...
        if "target_labels" in batch:
            new_labels = {}
            for name, labels in batch["target_labels"].items():
                new_labels[name] = labels.cuda(self.device)
