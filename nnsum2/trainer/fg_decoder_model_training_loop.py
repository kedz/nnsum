...
@Parameterized.register_object("trainer.fg_decoder_model_training_loop")
class FGDecoderModelTrainingLoop(Parameterized):
    ...
    def init_object(self):
        ...
        self._train_template = (
            ...
            + "d}/{:0" + TR_BATCHES_STRLEN +"d}  LOSS={:5.3f}  FG LOSS={:9.6f}"
        )       
        ...
        #model_path = model_dir / "model_checkpoints" / "{}.ckpt.{}.pth".format(
        #    model_dir.name, self.model_epoch)
        ...
        self.model.set_dropout(0)

        self._orig_weights = {name: param.clone().detach() 
    ...
    def _apply_fg_search_algos(self, batch, encoded_inputs):
        search_states = {}
        for name, search_algo in self.fg_search_algorithms.items():
            search_states[name] = search_algo["module"](
                self.model, batch, encoded_inputs=encoded_inputs)
        return search_states
    ...
    def _apply_loss_functions(self, forward_state, search_states, batch):
        loss = 0
        for lf in self.loss_functions.values():
            input_source = lf.get("input_source", "forward_state")
            if input_source == "forward_state":
                loss += lf["weight"] * lf["module"](forward_state, batch)
            elif input_source == "search_states":
                search_state = search_states[lf["input"]]
                loss += lf["weight"] * lf["module"](search_state, batch)
            else:
                raise Exception("Bad input source: {}".format(input_source))

        return loss

    def _apply_fg_loss_functions(self, search_states, batch):
        loss = 0
        for lf in self.fg_loss_functions.values():
            search_state = search_states[lf["input"]]
            loss += lf["weight"] * lf["module"](search_state, batch)
    ...
    def _epoch_results(self, avg_loss, mode):
        ...
        metric_results = {name: metric["module"].compute()
                          ...
                          if metric.get(mode, default)}
        all_results = {"loss_functions": loss_func_results,
                       "metrics": metric_results}
        if mode == "training": 
            fg_loss_func_results = {
                name: lf["module"].mean()
                for name, lf in self.fg_loss_functions.items()}
            all_results["fg_loss_functions"] = fg_loss_func_results
        return all_results
    ...
    def _train_epoch(self, batches):
        ...
        for step, (batch, teacher_batch) in enumerate(batches, 1):
            #if step > 100:
            #    continue
            ...
            loss = self._apply_loss_functions(forward_state, search_states,
                                              batch)

            encoded_inputs_teacher = self.model.encode(teacher_batch) 
            fg_search_states = self._apply_fg_search_algos(
                teacher_batch, encoded_inputs_teacher)
            fg_loss = self._apply_fg_loss_functions(
                fg_search_states, teacher_batch)
            ...
            #reg = 0
            #for name, param in self.model.named_parameters():
            #    reg += ((self._orig_weights[name] - param)**2).sum()

            self.model.train()
            # Perform the backward pass and update the model weights. 
            (loss + fg_loss).backward()
            self.optimizer.step()
            
            status = self._train_template.format(
                self.epoch, step, len(batches), total_loss / step, total_fg_loss / step)
